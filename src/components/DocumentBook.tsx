...
export default function DocumentBook({ invoice, onLoad }: { invoice: Invoice; onLoad: (v: Invoice) => void }) {
  ...
  return <details onToggle={refresh}><summary>{th ? "สมุดลูกค้า / สินค้า / ประวัติเอกสาร" : "Customers, products and document history"}</summary>
    <p className="field-hint">{th ? "บันทึกบนเบราว์เซอร์นี้เท่านั้น ไม่ซิงก์ข้ามเครื่อง สูงสุด 100 เอกสาร ส่งออกสำรองก่อนล้างข้อมูล" : "Stored only in this browser, not synced. Maximum 100 documents. Export a backup before clearing browser data."}</p>
    <div className="settings-history"><button type="button" onClick={() => act(() => saveContacts(invoice))}>{th ? "บันทึกผู้ขาย ลูกค้า และรายการนี้" : "Save contacts and products"}</button><button type="button" onClick={() => downloadFile("nexus-business-backup.json", JSON.stringify(readBook(), null, 2), "application/json")}>{th ? "สำรองข้อมูล JSON" : "Export backup JSON"}</button><button type="button" onClick={() => { if (confirm(th ? "ลบสมุดลูกค้าและสินค้า? ประวัติเอกสารยังอยู่" : "Clear contacts/products? Document history stays.")) act(clearContacts); }}>{th ? "ล้างสมุดข้อมูล" : "Clear address book"}</button></div>
